import { addDoc, deleteDoc, doc, getDocs, query, runTransaction } from "firebase/firestore";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { addGift, addGifts, claimGifts, getAvailableGifts, getGifts, removeGift } from "./gifts";
...
describe("gifts", () => {
  ...
  it("claimGifts throws when gift missing", async () => {
    ...
    await expect(claimGifts("inv-1", ["g1"], "Bob")).rejects.toThrow("Gift no longer exists");
  });
  ...
  it("claimGifts throws when already claimed", async () => {
    ...
    await expect(claimGifts("inv-1", ["g1"], "Bob")).rejects.toThrow("Gift already claimed");
  });
