...
describe("InviteList", () => {
  ...
  it("shows loading then invites", async () => {
    firestoreMocks.getInvitesByHost.mockImplementation(
      () => new Promise((resolve) => setTimeout(() => resolve([invite]), 50)),
    );
    render(<InviteList />);
    expect(document.querySelector(".animate-spin")).toBeTruthy();
    await waitFor(() => expect(screen.getByText("Summer Bash")).toBeInTheDocument());
  });
  ...
  it("toasts on delete failure", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /delete/i }));
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("Failed to delete invite"));
  });
  ...
  it("adds and removes gift in expanded section", async () => {
    ...
    const removeBtns = screen.getAllByRole("button").filter((b) => b.querySelector("svg.lucide-x"));
  ...
  it("toasts when add gift fails", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /^add gift$/i }));
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("Failed to add gift"));
  });
  ...
  it("toasts when remove gift fails", async () => {
    ...
    const removeBtns = screen.getAllByRole("button").filter((b) => b.querySelector("svg.lucide-x"));
    await user.click(removeBtns[removeBtns.length - 1]!);
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("Failed to remove gift"));
  });
  ...
  it("shows unknown template label fallback", async () => {
    ...
    render(<InviteList />);
    await waitFor(() => expect(screen.getByText("weird-type")).toBeInTheDocument());
  });
});
