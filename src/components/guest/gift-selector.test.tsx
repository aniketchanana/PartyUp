...
describe("GiftSelector", () => {
  it("returns null when no gifts", () => {
    const { container } = render(<GiftSelector gifts={[]} selectedIds={[]} onToggle={vi.fn()} />);
  ...
  it("toggles selection and shows link when present", async () => {
    ...
    render(<GiftSelector gifts={[gift, giftWithLink]} selectedIds={["g1"]} onToggle={onToggle} />);
  ...
  it("disables checkbox for claimed gifts", () => {
    ...
    render(<GiftSelector gifts={[claimed]} selectedIds={[]} onToggle={onToggle} />);
