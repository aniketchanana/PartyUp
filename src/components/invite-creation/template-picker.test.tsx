...
describe("TemplatePicker", () => {
  it("selects template, enables next, and calls back", async () => {
    ...
    const { rerender } = render(
      <TemplatePicker selected={null} onSelect={onSelect} onNext={onNext} onBack={onBack} />,
    );
    ...
    rerender(
      <TemplatePicker selected="birthday" onSelect={onSelect} onNext={onNext} onBack={onBack} />,
    );

    await user.click(screen.getAllByRole("button", { name: /next: gift registry/i })[0]);
